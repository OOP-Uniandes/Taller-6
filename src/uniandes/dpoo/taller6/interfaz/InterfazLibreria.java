...
public class InterfazLibreria extends JFrame {
	...
	public void aniadirCategoria() {
		String nombre = JOptionPane.showInputDialog(this, "Escriba el nombre de la categoría que desea añadir",
				"nombre");
		if (nombre != null) {

			Categoria categoriaEncontrada = libreria.buscarCategoria(nombre);

			if (categoriaEncontrada != null) {
				JOptionPane.showMessageDialog(this, "Ya existe una categoría con ese nombre", "Categoría existente",
						JOptionPane.INFORMATION_MESSAGE);
				return;
			} else {
				Categoria categoria = new Categoria(nombre, false);
				libreria.categorias[libreria.categorias.length] = categoria;
				JOptionPane.showMessageDialog(this, "Se ha creado la categoria", ":)",
						JOptionPane.INFORMATION_MESSAGE);
			}
	...
	// REQUERIMIENTO 3
	public void eliminarLibrosDeAutor() {
		String nombre = JOptionPane.showInputDialog(this,
				"Escriba el nombre de los autores (separados por ,) cuyos libros desea eliminar",
				"nombre");
		if (nombre != null) {

			String[] autores = nombre.replace(" ", "").split(",");

			Boolean hayLibrosPorCadaAutor = true;
			for (int i = 0; i < autores.length; i++) {
				ArrayList<Libro> librosDelAutor = libreria.buscarLibrosAutor(autores[i]);
				if (librosDelAutor.size() == 0) {
					hayLibrosPorCadaAutor = false;
				}
			}

			if (hayLibrosPorCadaAutor) {
				// eliminar los libros de los autores
				for (int i = 0; i < autores.length; i++) {
					ArrayList<Libro> librosDelAutor = libreria.buscarLibrosAutor(autores[i]);
					for (int j = 0; j < librosDelAutor.size(); j++) {
						libreria.eliminarLibro(librosDelAutor.get(j).darTitulo());
					}
				}
			}
